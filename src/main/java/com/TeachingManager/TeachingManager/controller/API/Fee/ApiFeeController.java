...
import com.TeachingManager.TeachingManager.DTO.Fee.FeeUpdateResponse;
...
public class ApiFeeController {
    ...
    // 수강 납부 or 수정 API
    @PutMapping("/api/fee")
    public ResponseEntity<FeeUpdateResponse> purchaseFee(@AuthenticationPrincipal CustomUser user
            , @RequestParam(value = "year") Short year
            , @RequestParam(value = "month") Short month
            , @RequestParam(value = "enroll_id") Long enroll_id
            , @RequestParam(value = "paid_amount") int paid_amount) {
        if(user != null && user.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_PRESIDENT"))) {
            return ResponseEntity.ok().body(new FeeUpdateResponse(feeService.purchaseLectureFee(user, enroll_id,paid_amount,year,month)));
        }
        return ResponseEntity.badRequest().build();
    }
}
